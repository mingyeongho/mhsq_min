<script lang="ts">
  ...
  $: 현재_퍼널_스텝 = $page.url.searchParams.get(qs); // 0 ~ steps.length
  $: 보여줄_퍼널_페이지 = 현재_퍼널_스텝 ? +현재_퍼널_스텝 : 0; // 0 ~ (steps.length - 1)

  $: 보여줄_퍼널 = steps[보여줄_퍼널_페이지];
  ...
  const 다음페이지로_이동 = ({
    ...
    const max_length = steps.length; // 37
    if (보여줄_퍼널_페이지 + 1 < max_length) {
